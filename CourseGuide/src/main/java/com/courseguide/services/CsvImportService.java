...
/**
 * Imports LLM-generated CSV into MySQL, tolerating row errors.
...
public class CsvImportService {
    ...
    public ImportResult importCoursePlan(String csvPath) {
        ImportResult result = new ImportResult();
        try {
            if (csvPath == null || csvPath.isBlank() || !Files.exists(Path.of(csvPath))) {
                result.addError("CSV path missing or not found: " + csvPath);
                return result;
            }
            ...
            List<Course> rows = CSVUtils.parseCoursesFromCSV(csvPath);
